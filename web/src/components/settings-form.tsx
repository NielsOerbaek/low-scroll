...
export function SettingsForm() {
  ...
  async function toggleFb(enabled: boolean) {
    setFbEnabled(enabled);
    await fetch("/api/settings", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ fbEnabled: enabled }),
    });
  }
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Install the Chrome extension to automatically sync your cookies
          </p>
        </div>
          ...
          <CardContent className="pt-6 space-y-4">
            <div className="space-y-3">
              ...
              <ol className="text-sm text-muted-foreground space-y-1.5 list-decimal list-inside">
                ...
                <li>
                  Open <code className="px-1 py-0.5 bg-muted rounded text-xs">chrome://extensions</code> and enable <strong>Developer Mode</strong>
                </li>
                <li>
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                Click the extension icon in Chrome, enter your instance URL and the API key below, then click Sync. The extension will read your Instagram cookies and send them securely to your low-scroll instance.
              </p>
            </div>
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Configure your Instagram session cookies
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Paste cookies manually, or use the browser extension above to sync automatically.
            </p>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Include posts from Facebook groups in your digest
            </p>
          </div>
          <Switch checked={fbEnabled} onCheckedChange={toggleFb} />
        </div>
        ...
        {fbEnabled && (
          <div className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  Sync Facebook cookies using the Chrome extension. Required cookies: <code className="px-1 py-0.5 bg-muted rounded text-xs">c_user</code>, <code className="px-1 py-0.5 bg-muted rounded text-xs">xs</code>.
                </p>
                {settings.hasFbCookies && (
                  <Button
                    variant="outline"
      ...
      {/* ── Section: Digest Settings ───────────────────────────── */}
      <section className="space-y-3">
        <div>
          <h2 className="text-lg font-semibold">Digest Settings</h2>
          <p className="text-sm text-muted-foreground">
            Configure when and where to receive your digest email
          </p>
        </div>
